...
- Multiple vehicles, each node has a time windows for delivery
	...
	- Hard: time windows _must_ be satisfied exactly (typical case)
...
So the result is _not_ one continuous tour by one truck with repeated depot visits, but it models a second, third ... vehicle starting also at t=0
