...
class InferenceSolver:
    ...
    def solve_batch(
        ...
        *,
        ...
        self.model.eval()
        self.env.load_problems(episode, batch_size, **load_kwargs)
        origin_problem = self.env.problems
        if self.env.solution is not None:
            baseline_length = self.env.compute_tour_length(origin_problem, self.env.solution)
    ...
    def _run_greedy(self, batch_size, reencode_each_step=False):
        ...
        while not done:
            if current_step == 0:
                reference = torch.zeros(batch_size, dtype=torch.long, device=self.device)
                predicted = reference
            else:
                output = self.model(
                    state,
                    self.env.reference_tour,
                    self.env.solution,
                    current_step,
        ...
        return self.env.reference_tour
    ...
    def _run_beam(self, batch_size, problem_size, reencode_each_step=False):
        ...
        self.env.reset(batch_size * self.beam_size)
        state, _, _, done = self.env.pre_step()
        self.model.prepare_instance(state=state)
        current_step = 0
        while not done:
            if current_step == 0:
                selected = torch.zeros(batch_size * self.beam_size, dtype=torch.long, device=self.device)
            else:
                output = self.model(
                    state,
                    self.env.reference_tour,
                ...
                probs[probs.isnan()] = 0
                self.env.reference_tour = beam.advance(probs, self.env.reference_tour)
                selected = beam.next_nodes[-1].view(-1)
            state, tour_lengths, done = self.env.step_beam(selected, beam=self.beam_size)
            current_step += 1

        tour_lengths = tour_lengths.view(batch_size, self.beam_size)
        beam_tours = self.env.reference_tour.view(batch_size, self.beam_size, -1)
    ...
    @torch.no_grad()
    def run_beam_on_coordinates(self, episode, batch_size, problem_size):
    ...
    @torch.no_grad()
    def run_greedy_on_coordinates(self, episode, batch_size):
