...
class LocalDecoder(nn.Module):
    ...
    def _select_local_candidates(
        self, data, selected_tour, problem_size, batch_size, dis_matrix, beam_search=False, beam_size=16
    ):
            ...
            if num_remaining <= K_NEAREST_NEIGHBORS:
                ...
                if beam_search:
                    ...
                    distance = dis_matrix.gather(
                        1, last_node.view(effective_batch, 1, 1).expand(-1, -1, problem_size)
                    ).squeeze(1)
    ...
    def _local_distance_matrix(self, unselected_nodes, dis_matrix, batch_size, beam_search, beam_size, problem_size):
        ...
        if beam_search:
            ...
            return expanded[index_1, index_2, index_un, index_un.transpose(2, 3)].view(
                batch_size * beam_size, unselected_nodes.shape[-1], unselected_nodes.shape[-1]
            )
    ...
    def forward(self, data, selected_tour, dis_matrix, beam_search=False, beam_size=16):
        ...
        local_embeddings, unselected_nodes = self._select_local_candidates(
            data, selected_tour, problem_size, batch_size, dis_matrix, beam_search, beam_size
        )
        first_and_last = self._gather_node_embeddings(
            data, selected_tour[:, [0, -1]], beam_search=beam_search, beam_size=beam_size
        )
        embedded_first = self.first_node_embedding(first_and_last[:, 0])
        embedded_last = self.last_node_embedding(first_and_last[:, 1])
        decoder_input = torch.cat(
            (embedded_first.unsqueeze(1), local_embeddings, embedded_last.unsqueeze(1)), dim=1
        )
        ...
        local_dist = self._local_distance_matrix(
            unselected_nodes, dis_matrix, batch_size, beam_search, beam_size, problem_size
        )
        out = decoder_input
