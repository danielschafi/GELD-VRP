...
class LocalDecoder(nn.Module):
    ...
    def _select_local_candidates(
        ...
        remaining_mask = torch.arange(problem_size, device=data.device)[None, :].repeat(
            effective_batch, 1
        )
        selected_indices = selected_tour.type(torch.long)
        batch_indices = torch.arange(
            effective_batch, dtype=torch.long, device=data.device
        )[:, None].expand(effective_batch, selected_indices.shape[1])
        remaining_mask[batch_indices, selected_indices] = -2
        num_remaining = problem_size - selected_tour.shape[1]

        if self.training:
            unselected = remaining_mask[torch.gt(remaining_mask, -1)].view(
                batch_size, num_remaining
            )
        else:
            if num_remaining <= K_NEAREST_NEIGHBORS:
                unselected = remaining_mask[torch.gt(remaining_mask, -1)].view(
                    effective_batch, num_remaining
                )
            else:
                last_node = selected_tour[:, -1]
                if beam_search:
                    last_node = last_node.view(batch_size, beam_size, 1).expand(
                        -1, -1, problem_size
                    )
                    distance = dis_matrix.gather(1, last_node).view(
                        batch_size * beam_size, -1
                    )
                else:
                    distance = dis_matrix.gather(
                        1,
                        last_node.view(effective_batch, 1, 1).expand(
                            -1, -1, problem_size
                        ),
                    ).squeeze(1)
                mask = torch.zeros_like(distance)
                mask[batch_indices, selected_tour] = 1e2
                unselected = torch.topk(
                    distance + mask, dim=1, k=K_NEAREST_NEIGHBORS, largest=False
                ).indices
        ...
        if beam_search:
            ...
            local_embeddings = torch.gather(
                expanded_data,
                2,
                unselected.view(batch_size, beam_size, -1, 1).expand(
                    -1, -1, -1, emb_dim
                ),
            )
            ...
            local_embeddings = torch.gather(
                data, 1, unselected.unsqueeze(2).expand(-1, -1, emb_dim)
            )
        return local_embeddings, unselected

    def _gather_node_embeddings(
        self, encoded_nodes, node_index, beam_search=False, beam_size=16
    ):
        ...
        if beam_search:
            ...
            gathering_index = node_index.view(
                batch_size // beam_size, beam_size, num_selected, 1
            ).expand(batch_size // beam_size, beam_size, num_selected, embedding_dim)
            picked = encoded_nodes.gather(dim=2, index=gathering_index)
            return picked.view(batch_size, num_selected, embedding_dim)
        gathering_index = node_index[:, :, None].expand(
            batch_size, num_selected, embedding_dim
        )
    ...
    def _local_distance_matrix(
        ...
        if problem_size > LARGE_INSTANCE_THRESHOLD:
            if beam_search:
                index_un = unselected_nodes.view(batch_size, beam_size, -1, 1).expand(
        ...
        if beam_search:
            ...
            expanded = dis_matrix.unsqueeze(1).expand(
    ...
    def forward(self, data, selected_tour, dis_matrix, beam_search=False, beam_size=16):
        ...
        full_probs = (
            torch.zeros(output_batch_size, problem_size, device=out.device) + 1e-5
        )
        batch_indices = torch.arange(
            output_batch_size, dtype=torch.long, device=out.device
        )[:, None]
