...
class VRPTWEnv:
    ...
    def load_problems(self, batch_size, problems=None, aug_factor=1):
        if problems is not None:
            ...
            depot_xy, node_xy, node_demand, service_time, tw_start, tw_end = self.get_random_problems(
                batch_size, self.problem_size, normalized=True
            )
        ...
        self.reset_state.prob_emb = (
            torch.FloatTensor([1, 0, 0, 0, 1]).unsqueeze(0).to(self.device)
        )  # bit vector for [C, O, B, L, TW]

        self.step_state.BATCH_IDX = self.BATCH_IDX
        self.step_state.POMO_IDX = self.POMO_IDX
        self.step_state.open = torch.zeros(self.batch_size, self.pomo_size).to(self.device)
        self.step_state.START_NODE = (
            torch.arange(start=1, end=self.pomo_size + 1)[None, :].expand(self.batch_size, -1).to(self.device)
        )
    ...
    def reset(self):
        ...
        # shape: (batch, pomo)
        self.visited_ninf_flag = torch.zeros(size=(self.batch_size, self.pomo_size, self.problem_size + 1)).to(
            self.device
        )
        # shape: (batch, pomo, problem+1)
    ...
    def step(self, selected):
        ...
        self.current_time = (
            torch.max(
                self.current_time + new_length / self.speed,
                self.depot_node_tw_start[torch.arange(self.batch_size)[:, None], selected],
            )
        ...
        arrival_time = torch.max(
            self.current_time[:, :, None]
            + (
                self.current_coord[:, :, None, :] - self.depot_node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)
            ).norm(p=2, dim=-1)
            / self.speed,
            self.depot_node_tw_start[:, None, :].expand(-1, self.pomo_size, -1),
        )
        out_of_tw = (
            arrival_time > self.depot_node_tw_end[:, None, :].expand(-1, self.pomo_size, -1) + round_error_epsilon
        )
        # shape: (batch, pomo, problem+1)
        ...
        fail_return_depot = (
            ...
            + (
                self.depot_node_xy[:, None, :1, :]
                - self.depot_node_xy[:, None, :, :].expand(-1, self.pomo_size, -1, -1)
            ).norm(p=2, dim=-1)
            / self.speed
            > self.depot_end + round_error_epsilon
        )
        # shape: (batch, pomo, problem+1)
    ...
    def load_dataset(self, path, offset=0, num_samples=1000, disable_print=True):
        ...
        depot_xy, node_xy, node_demand, capacity, service_time, tw_start, tw_end = (
            [i[0] for i in data],
            [i[1] for i in data],
            [i[2] for i in data],
            [i[3] for i in data],
        ...
        depot_xy, node_xy, node_demand, capacity, service_time, tw_start, tw_end = (
            torch.Tensor(depot_xy),
            torch.Tensor(node_xy),
            torch.Tensor(node_demand),
            torch.Tensor(capacity),
            torch.Tensor(service_time),
            torch.Tensor(tw_start),
            torch.Tensor(tw_end),
        )
    ...
    def get_random_problems(
        ...
        a, b = (
            self.depot_start + travel_time,
            self.depot_end - travel_time - service_time,
        )
        time_centers = (a - b) * torch.rand(batch_size, problem_size) + b
        time_half_width = (service_time / 2 - self.depot_end / 3) * torch.rand(
        ...
        total_time = (
            torch.max(0 + (depot_xy - node_xy).norm(p=2, dim=-1) / self.speed, tw_start)
            + service_time
            + (node_xy - depot_xy).norm(p=2, dim=-1) / self.speed
            > self.depot_end + round_error_epsilon
        )
        # (batch, problem)
        ...
        if instance_invalid:
            ...
            node_demand = torch.Tensor(
                np.random.randint(1, 10, size=(batch_size, problem_size))
            )  # (unnormalized) shape: (batch, problem)
